...
Internal Intel: http://mjc.intel.com/xed
External World: https://software.intel.com/en-us/articles/xed-x86-encoder-decoder-software-library

Send bugs to mark.charney@intel.com

Download changes:

Before July 2015, XED users could obtain XED kits from within Pin
kits. After July 2015, Pin kits use their own C-runtime ("Pin CRT")
and the libxed.so that pin kits include is compiled against that
C-runtime. That makes the libxed in the pin kits not useful for people
not working with pin -- it won't work with users' system CRT.  The
links above provide XED kits that work with "normal" system-provided
C-runtimes that non-pin usages require.

The libxed in the new XED kits is built on SUSE10SP4 using a rather
old glibc 2.4.  Building against that old glibc version helps ensure
that libxed will work on more modern linux systems.
